...
let root=null,lastKey='',category='food',feedback='',pendingImport=null;
...
function levelPage(){
  ...
  return page(MOBILE_UI.text030,`${difficulty()}<ol class="level-list">${LEVELS.map((lv,i)=>{
    const open=lv.hidden?bridge.unlocked:i===0||save.cleared[i-1];
    const medals=save.medals[save.difficulty][i],modern=Object.values(medals).some(Boolean);
    return `<li class="level-card ${open?'':'locked'}"><span class="station">${String(i+1).padStart(2,'0')}</span><div><h2>${esc(lv.name)}</h2><p>${esc(lv.sub)}</p><small>${modern?`${MOBILE_UI.text031}${starText(Object.values(medals).filter(Boolean).length)}`:`${MOBILE_UI.text032}${starText(save.stars[i])}`}</small>${!open?`<p class="lock-note">${lv.hidden?`${MOBILE_UI.text033}${bridge.starCount}/15 ${MOBILE_UI.text034}${bridge.ordinaryCount}/28 ${MOBILE_UI.text035}`:`${MOBILE_UI.text036}${esc(LEVELS[i-1].name)}${MOBILE_UI.text037}`}</p>`:''}</div>${open?btn('level:'+i,MOBILE_UI.text038,'small-button',`aria-label="${MOBILE_UI.text039}${esc(lv.name)}"`):MOBILE_UI.text040}</li>`;
  }).join('')}</ol>`);
}
function albumPage(){
  if(G.albumZoom){
...
function shopPage(){
  ...
  }).join('')}</div><section class="skin-choice"><h2>${MOBILE_UI.text059}</h2><p>${MOBILE_UI.text060}</p>${btn('skin:white',MOBILE_UI.text061,save.selectedSkin==='white'?'selected':'secondary',`aria-pressed="${save.selectedSkin==='white'}"`)}${btn('skin:gold',gold?MOBILE_UI.text062:MOBILE_UI.text063,save.selectedSkin==='gold'?'selected':'secondary',`${gold?'':'disabled'} aria-pressed="${save.selectedSkin==='gold'}"`)}</section>`);
}
...
function paintIllustrations(){
  ...
  for(const canvas of root.querySelectorAll('canvas[data-duck]'))drawBookDuck(canvas.getContext('2d'),150,280,205,{t:1});
}
export function renderDomUI(){
  if(!root)return;
  const hero=root.querySelector('[data-duck]');
  if(hero&&!hero.dataset.loaded&&getSprite('bookDuck')){hero.getContext('2d').clearRect(0,0,hero.width,hero.height);drawBookDuck(hero.getContext('2d'),150,280,205,{t:1});hero.dataset.loaded='true';}
  const playing=G.state==='play'||G.state==='crashing';
  const key=playing?'':JSON.stringify([G.state,G.paused,G.albumZoom,category,save.coins,save.selectedSkin,save.trackedItem,save.muted,save.difficulty,save.motion,save.ups,save.album,save.stars,feedback,!!pendingImport]);
  // 局内只更新文本，不按鸭蛋数量替换 DOM，保证触控和焦点稳定。
  const stateKey=playing?G.state+':'+G.paused:key;
  if(lastKey!==stateKey){
    const oldState=lastKey.split(':')[0];lastKey=stateKey;
    ...
    else if(!playing&&oldState!==stateKey)root.querySelector('h1')?.focus({preventScroll:true});
  }
  if(playing){
...
function go(state){G.state=state;G.albumZoom=null;feedback='';pendingImport=null;}
...
function handle(action){
  ...
  else if(id==='skin'&&(value==='white'||save.stars.reduce((a,b)=>a+b,0)>=15)){save.selectedSkin=value;persist();}
...
export function mountDomUI(){
  root=document.getElementById('app-ui');lastKey='';
