...
当前候选：换鸭、星级指引与全图鉴照片优化版（2026-09-07）。竖屏主玩、横屏兼容，原生 HTML 界面；十关都有独立六段编排、两档难度、收集目标与两枚技巧印章，场景和角色统一为绘本风格。原标杆已获用户 iPhone 体验认可；本轮 iPhone 换鸭复核已获用户「切换正常」反馈；浏览器模拟与实机记录分别保留。更新说明见 [EXPERIENCE-UPDATE.md](EXPERIENCE-UPDATE.md)，照片逐项结果见 [ALBUM-REVIEW.md](ALBUM-REVIEW.md)。执行状态见 [MOBILE-UPGRADE.md](MOBILE-UPGRADE.md)，入口见 [PLAYTEST.md](PLAYTEST.md)。
...
南京城市主题跑酷小游戏:跑遍金陵十景(明城墙、玄武湖、中山陵、夫子庙、紫金山、颐和路、老门东、栖霞山、大报恩寺 + 隐藏关「长江大桥」),收集 40 件金陵风物(食/工/迹/灵四部,含 6 件按条件解锁的隐藏档)。前九关全部通关、前九关至少 15 星且收集 28/34 件普通风物后解锁大桥,旧档已经通桥则继续开放。十站和无尽采用暖金湖青绘本场景与后视桂花鸭；桌面局内角色按用户反馈等比收小 25%。程序化画面负责装饰载入失败时的后备。无框架、无打包器、原生 ES Modules；源码可直接调试，正式发布使用版本化 `dist/` 制品。

玩法:三车道跑酷,低障碍跳、高障碍滑铲、整墙换道;连击有音阶爬升,无尽模式有里程碑勋章与 600m 地标报站;拾取的鸭蛋可在鸭铺升级局内道具(磁铁/护盾/金桂);结算页可生成拍立得成绩卡分享;各关历史最好星级之和达到 15 星解锁金鸭，不消耗星数；每关最多 3 星，跨难度不重复计算。
