...
现役分支为 `codex/trusted-experience`，正式站点为 EdgeOne Pages 上的 `https://jinlingrun.caowenhu.com/`，源码仓库保持私有。本轮正式构建为 `b599cfb77d82`，上一构建 `18741739d7da` 的资源已保留。

本轮已完成关卡递进、首页难度与目标、教学失败边界、技巧判定和一致文案、磁铁吸附、撞击动画、结算图鉴往返，以及多模型署名与素材版权页同步。用户已确认本轮同步、部署、上线。正式 CI `34108877618`（来源 `59d42db`）与20分钟持续运行检查均已通过，EdgeOne 部署 `dp9a2abv7fw2` 成功，公网新旧210个版本文件已逐字节核验；具体证据与上线版本以 `docs/release/publish-status.md` 为准。

## 2026-09-02 收口历史快照（非当前状态）
