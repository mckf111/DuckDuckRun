...
现役分支为 `codex/trusted-experience`，正式站点为 EdgeOne Pages 上的 `https://jinlingrun.caowenhu.com/`，源码仓库保持私有。此前已上线构建为 `18741739d7da`。

本轮已完成关卡递进、首页难度与目标、教学失败边界、技巧判定和一致文案、磁铁吸附、撞击动画、结算图鉴往返，以及多模型署名与素材版权页同步。用户已确认本轮同步、部署、上线。接下来以最终 main 提交的正式 CI 制品与20分钟持续运行检查为发布依据，保留上一版资源；具体进度与上线版本以 `docs/release/publish-status.md` 为准。
