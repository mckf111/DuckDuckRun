...
| `src/game.js` | 状态机、生成、教程、碰撞、结算、重开和隐藏风物安全路线 | 保留；固定 seed 重开重置玩法随机流，保证风物生成保留安全区 |
| `src/rules.js` | 纯玩法规则、星级、解锁判断 | 可单元测试的良好边界 |
| `src/input.js` | 可创建/销毁的键盘、Pointer/touch、窗口失焦与可见性暂停控制器 | 唯一输入语义；监听器由控制器成对注册与释放 |
| `src/render.js` 与 `src/art/` | Canvas 场景与精灵绘制、照片加载/预取 | 保留视觉边界；背景预取可取消，加载失败继续使用代码绘制降级背景 |
...
| `tools/e2e/` | 语法、Playwright 浏览器回归、制品 404 与体积预算检查 | 覆盖 A-01 契约、启动/重开/输入/resize/音频/seed，使用锁定的浏览器工具依赖 [3] |
...
| `docs/qa/` | 设备/浏览器矩阵、工程加固报告与机器可读素材 SBOM | 区分模拟与真机，不得混写；默认被 `.gitignore` 忽略，提交时必须显式加入 |
...
该项目没有打包器或运行时框架；`package.json` 锁定 `npm@10.9.2`，根 `package-lock.json` 仅锁定零依赖项目本身，浏览器测试依赖在 `tools/e2e/package-lock.json` 锁定 [4]。`npm run build` 是零依赖静态复制/版本化过程，不改变 Canvas/ESM 运行架构。后续工具只能服务于已定义的质量需求，不能改变该默认架构。

## 文档承接规则

审查文档中的问题编号（A-01 至 A-11）是后续变更的唯一风险主键。阶段 2 的运行时、测试、工作流和 SBOM 修改均对应 A-01 至 A-09；提交说明必须列出相应编号，并更新 `MEMORY.md`。截图/原始日志不随本阶段提交；测量口径、命令和结论由 `docs/qa/engineering-hardening-report.md` 管理。
