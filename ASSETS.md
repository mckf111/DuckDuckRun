...
下列“当前”“下一阶段”均指各章节当时的状态，不覆盖上方 2026-09-07 更新。
...
新增三张 1536×1024 横向三联图集 `book-journey-1/2/3.webp`，运行时按关卡读取对应竖幅，合计 997618 字节。依次覆盖玄武湖／中山陵／夫子庙、紫金山／颐和路／老门东、栖霞山／大报恩寺／长江大桥。生成使用本项目 book-wall.webp 作为画风参考，不使用摄影或商标参考图。完整提示词、输入参考、原始 PNG 和哈希存于私有证据 `docs/mobile-upgrade/private-evidence/journey-images.json`。属于游戏化场景，不是建筑考据或景区授权素材。

2026-09-06 的完整台账为 58 项，均保留待审状态。字体覆盖十关新文案；分享卡与无尽模式复用统一角色、对应场景。当前图集只按进入关卡加载，首页仍仅加载城墙与鸭子图集。

## 2026-09-07 发布复核

当前工程台账为 88 项，其中随网页分发的 70 项已按 SHA-256 完成工程发布复核，18 项停用照片只留历史追溯。具体决定见 `assets/release-review.json` 和 [复核报告](docs/release/asset-review-2026-09-07.md)。这替代上面的历史“全部待审”状态；不冒充法律意见或真人测试。

本轮找回了旧鸭子、障碍和拾取光环的原始生成文件。图集/光环用原处理脚本重建后与仓库字节一致；图标从鸭子图集派生。用户确认 Grok 使用个人订阅，官方 Consumer FAQ 允许输出用于商业用途。相应素材保留 **Created with Grok** 署名；当前绘本素材按 OpenAI 来源登记，南京切片部分素材按 Manus 来源登记。

开发整体由作者组织多个工具接力。用户补充大致由 Kimi 起步、Grok 参与，可能使用过 Qwen，最后交由 Codex 承接；不确定的模型版本不作为逐文件归属结论。
