...
通用协作约定沿用全局 AGENTS.md；以下保留本项目约束。

## 定位

《冲鸭！金陵！》：南京城市主题三车道跑酷 H5（逃出鸭店的白胖鸭跑遍金陵十景，收集 40 件风物图鉴，含 6 件隐藏档）。源码可通过 HTTP 直接运行，无框架和 npm 运行时依赖；正式发布使用通过 CI 的版本化静态制品。画面为 AI 辅助绘本、程序化插画与 Wikimedia Commons 实景照片。游戏本体保留所有权利，见 `LICENSE.md`。工作只在 `codex/trusted-experience` 进行，不直接改 `main`。
...
原生 JS ES Modules + Canvas 2D（960×540 逻辑分辨率，DPR 适配）、WebAudio 合成音频、localStorage 存档（key=`jinling_run_v1`）。无框架、无 npm 运行时依赖（仅 tools/e2e 用 playwright-core）；静态构建负责指纹化、复制和版本化发布。
...
现役托管为 EdgeOne Pages 的 `jinlingrun` 项目，正式域名 `https://jinlingrun.caowenhu.com/`，仓库保持私有。发布步骤见 `docs/deployment/edgeone.md`，现役记录见 `docs/release/publish-status.md`；下方快照不作为当前发布事实。

## 历史快照（2026-08-04，非当前状态）

以下提交、分支与待办只记录当时状态。恢复工作时先核对现场与用户当前授权，不据此宣称今天已完成或仍待办；上文项目红线继续适用。
